...
if (isset($_POST['upload'])) {
    ...
    if (move_uploaded_file($videoTmpName, $videoPath)) {
        // Insert video details into the database (with description)
        $stmt = $conn->prepare("INSERT INTO videos (video_name, video_path, description) VALUES (?, ?, ?)");
        $stmt->bind_param("sss", $videoName, $videoPath, $description);
        $stmt->execute();
...
// Handle video deletion (only from the database)
if (isset($_GET['delete'])) {
    ...
    if ($stmt->affected_rows > 0) {
        echo "Video record deleted successfully!";
    } else {
        echo "Error deleting video record!";
    }
  ...
  <div class="container-fluid bg-primary py-5 mb-5 page-header">
    ...
    <!-- Header End -->
     ...
     <form action="" method="POST" enctype="multipart/form-data">
                    ...
                    <textarea name="description" placeholder="Enter video description" required></textarea><br><br>

                    <div class="text-center">
                        <button type="submit" class="btn btn-primary" name="upload">Upload Video</button>
                    </div>
                </form>
...
while ($row = $result->fetch_assoc()) { 
    ...
    if ($counter % 2 == 0) {
        // Even rows: Video on the right, text on the left
        echo '        <div class="col-lg-6 col-12">';
        echo '            <div class="px-5 py-5 py-lg-0">';
      
        echo '                <h4><strong>Description:</strong> ' . htmlspecialchars($row['description']) . '</h4>';
        echo '                <a href="?delete=' . $row['id'] . '">Delete</a>';
        echo '            </div>';
        echo '        </div>';

        echo '        <div class="col-lg-6 col-12">';
        echo '            <video width="600" height="409" id="videoPlayer" controls="controls">';
        echo '                <source src="' . $row['video_path'] . '" type="video/mp4">';
        echo '            </video>';
        echo '        </div>';
    } else {
        // Odd rows: Video on the left, text on the right
        echo '        <div class="col-lg-6 col-12">';
        echo '            <video width="600" height="409" id="videoPlayer" controls="controls">';
        echo '                <source src="' . $row['video_path'] . '" type="video/mp4">';
        echo '            </video>';
